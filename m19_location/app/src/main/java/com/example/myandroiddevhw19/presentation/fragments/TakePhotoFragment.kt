package com.example.myandroiddevhw19.presentation.fragments
...
import com.example.myandroiddevhw19.R
import com.example.myandroiddevhw19.data.model.Photo
import com.example.myandroiddevhw19.databinding.FragmentTakePhotoBinding
import com.example.myandroiddevhw19.presentation.ViewModelFactory
import com.example.myandroiddevhw19.presentation.viewmodels.TakePhotoViewModel
...
class TakePhotoFragment : Fragment() {
    ...
    private fun clickButtonListPhoto() {
        binding.buttonGoToSavedPhoto.setOnClickListener {
                ...
                object : ImageCapture.OnImageSavedCallback {
                    override fun onImageSaved(outputFileResults: ImageCapture.OutputFileResults) {
                        Glide.with(requireActivity())
                            ...
                            .fitCenter()
                            .into(binding.image)
                        val datePhoto = SimpleDateFormat(FILE_NAME_FORMAT, Locale.getDefault())
                            .format(System.currentTimeMillis())
                        val photo = Photo(viewModel.getUriLastPhoto(requireContext()), datePhoto)
                        viewModel.insertPhoto(photo)
                        Toast.makeText(
                            requireContext(),
                            getString(R.string.photo_saved_text), Toast.LENGTH_SHORT
                        ).show()
